...
\usepackage{url, fancyvrb, newverbs, xcolor, tcolorbox, listings}

%packes to add - realbox, minted
...
% Make it so that inline verbatim text behaves similarly to the verbatim chunks defined above
\newcommand\ilcodewrap{\adjustbox{cfbox=background}}
...
To see if the consensus contruction, variant insertion, and masking worked, I wrote a script (``6.3-GetGenomeAlignments'') to clip a stretch of bases off the first scaffold of each accession. This script will probably be a useful starting point for when I want to divide the genome up into windows for phylogenetic inference. The results of this clipping were visualized in SeaView version 5 \cite{galtier1996}. In general, the bam-centric sequence reconstruction process has produced long stretches of good-looking alignments. Of course, the ``cleaness'' of this alignment may be expected due to the consensus process used to generate full species sequences.
 ...
 \begin{figure}[!h]
         ...
         \caption{An example of low diversity genomic stretches which are common throught the multiple species alignment (MSA), at least for this considered genomic region.}
 \end{figure}
...
\begin{codewrap}
   sed '/>/\! s/N/-/g' alignment.fa > alignment_test.fa
\end{codewrap}
to make a temporary file in which Ns (those which are not present in fasta headers) are converted to the hyphen gap character. The temporary file is then dumped at the end of the trimAl run. This is not particularly computationally efficient, and it could be more viable to just assign the hyphen instead of an N as a gap character during initial consensus genome construction (in script ``''). \\
This seems to have allowed trimAl to run properly. However, for some reason, the trimAL parameter file (made in script ``'') throws up an error when launcing a queued job on the cluster. For now, porting the contents of the parameter file to a new file in the home directory has fixed the issue. \\
In the process of organizing this, I've come across a misconception of mine (with the help of Maya Gueguen). I previously considered that when running an array job, the \verb!${1}! vriable refers to the entirity of the current line in the parameter file. Instead, it refers to the first item in the current line of the considered parameter file (with white-space separation). This means a great deal of my script made to loop through independent alignment files in each line were unnecessary. However, looping through these parameters provides a hidden challenge, one that can be circumvented with some of the ``indirection expansion'' approaches used in a post on StackOverflow (\url{https://stackoverflow.com/questions/37053780/iterate-over-arguments-in-a-bash-script-and-make-use-of-their-numbers}).

\section*{Thursday, 5 May 2022}
\subsection*{\underline{Reconfiguring ANGSD}}

The initial ANGSD script (``4.2-GenotypingANGSD.sh'') was quite strict. Variant positions were only retained if found across at least 32 (out of a possible 40) samples (-minInd 32) and if SNPs were highly supported (-SNP\_pval 1e-6). In reality, we have a great deal of missing data for more diverged samples (in relation to the reference), meaning that these criteria may be limiting our detection of variants between accessions. Accordingly, these parameters have been changed as follows: -minInd 1; -SNP\_pval 1. Along with increased sensitivity to SNPs or fixed differences amongst samples, this approach will increase the likelihood of retaining sequencing errors. To offset this risk, the minimum depth with which a genotype at a site must be supported has been increased from 3 to 5 (-setMinDepthInd option).

After running the adapted script, ANGSD returned only 827 000 SNP loci, fewer than the 1.3 million loci recovered previously. I have tweaked the script again to completely discount the SNP \emph{p}-value to see if it makes a difference.
Without the SNP cut-off, roughly 1.4 million loci are recovered. This is curious, as it was expected that every recoverable genotype should be returned. For example, with high mapping coverage accessions (such as those belonging to \emph{C}. \emph{arcania}), I presumed that the genotype for every mapped base (which would amount to roughly 80\% of the 600 million bases of the reference genome) would be recovered by ANGSD. The parameters should be looked at again. I can imagine that the 1) the genotype likelihood option set to 0.9 is quite strict, and/or that 2) matches to the reference genome are not returned as called sites. The second option would be perculiar though, as the polymorphism rate across the genome of \emph{Coenonympha} would end up extremely low ((1400000/600000000)*100 = 0.2\%).

I wanted to test the script again, as overall I have noted that variant density can be quite low in some areas. As an example, in the presumed mitochondrial region - scaffold\_109 (see entry on 22 March 2022) - there are only two informative/variable sites amongst all of my accessions. In the original ANGSD script, there is a cut-off sequence depth value in place to avoid the genotyping of potential paralogous sites. This could be limiting the detection of sites in mtDNA, which are typically present at high copy numbers in each individual. In addition, the GATK model (add reference!; option \verb!GL 2! in ANGSD) has been used for calculating genotype likelihoods. However, GATK seems to underperform when analysing non-human genomes (references!) - I have therefore attempted to swap to the SAMtools genotype likelihood model (reference; option \verb!GL 1! in ANGSD). These changes, in addition to those listed in the above paragraphs of this journal entry have been captured in a new script, entitled ``4.2-GenotypingANGSD\_relaxed.sh''.

The above-described script has yielded roughly 30 million genotyped positions across all accessions, with 58 000 - 2 500 000 genotypes being reported per accession (see Figure 5). Counts of recovered genotypes were obtained by pulling the number of applied variants from the output files specified in the ``6.2-GetConsensusGenomesBCFTools.sh'' script as follows:

\begin{codewrap}
  # Use awk to pull the second white-space delimited field of the standard output generated by the bcftools consensus command. The output routes into the standard error file and contains only a single line, in the form ``Applied {a number} variants''.
  awk '{print $2}' 6.2*err > bcftools_consensus_applied_variants.csv
